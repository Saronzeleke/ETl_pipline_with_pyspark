...
class DataExtractor:
    ...
    "local": config.RAW_DATA_DIR / "nyc_311_sample_100k.csv"
}
,
    ...
    def _create_spark_session(self) -> SparkSession:
        spark = (
            ...
            .master("local[*]")
            .config("spark.driver.memory", config.SPARK_CONFIG.get("spark.driver.memory", "1g"))
            .config("spark.executor.memory", config.SPARK_CONFIG.get("spark.executor.memory", "1g"))
    ...
    def _download_file(self, url: str, local_path: Path):
        ...
        try:
            with requests.get(url.strip(), stream=True, timeout=30) as r:
                r.raise_for_status()
    ...
    def extract_all_sources(self) -> dict:
        ...
        logger.info(f"✅ Loaded Parquet: {len(trips_df.columns)} columns, {trips_df.count()} rows")
        ...
        logger.info(f"✅ Loaded CSV: {len(customers_df.columns)} columns, {customers_df.count()} rows")

        # 3️⃣ JSON - Weather Data
        w = self.sources["weather"]
        self._download_json(w["url"], w["local"])
        weather_df = self.spark.read.option("multiLine", True).json(str(w["local"]))
        logger.info(f"✅ Loaded JSON: {len(weather_df.columns)} columns, {weather_df.count()} rows")
